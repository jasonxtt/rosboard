...
- `trusted_proxy_cidrs` is a startup-only process-global security boundary. It
  contains only immediate reverse-proxy peers allowed to provide sanitized
  `X-Forwarded-Proto` / `X-Forwarded-Host` values; a trusted proxy must
  provide one `X-Forwarded-Proto` value, while `X-Forwarded-Host` may be
  omitted when the public Host is preserved; an empty list preserves direct-
  connection behavior.
...
- Config: trusted proxy CIDRs load, persist, reject malformed values, and are
  not exposed through the panel settings projection.
...
- MosDNS fields (per device): `mosdns.enabled`, `mosdns.baseUrl`, `mosdns.syncIntervalMinutes` plus the live sync counters (`lastImported`, `learnedFeatureCount`, ...). Synchronization is disabled by default, leaves `baseUrl` empty until the operator enters an address such as `10.0.0.3`, and then uses the normalized local HTTP endpoint at a 5-minute interval. The first (no-watermark) sync imports at most the most recent 24 hours of audit history; steady-state syncs page only up to the stored watermark. Learned IP features live in the owning device database. Attribution resolves connections from learned DNS fingerprints (`dns_features`), never from TTL-gated raw observations (pull-based sync lags far behind answer TTLs, so TTL gating starves the evidence): a fingerprint is usable while its `last_seen` is at most 7 days old, the newest fingerprint wins per `(client, answer IP)`, each sync prunes features older than 7 days, and `match_window_minutes` only splits the `mosdns` (in-window) versus `mosdns-learned` (older) source label.
