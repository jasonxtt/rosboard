...
- `connectedDeviceCount` includes only `state == online` LAN terminals. It excludes `routeros:self`, idle/offline terminals, selected traffic interfaces, loopback, and interfaces identified as WAN. An online terminal with no known interface remains countable so missing neighbor metadata does not hide a real device.
...
- RouterOS self/known WAN/idle/offline terminal -> exclude from `connectedDeviceCount`.
...
- Unit: connected LAN device count excludes RouterOS self, idle, offline, WAN, and selected traffic-interface terminals while retaining online unknown-interface terminals.
