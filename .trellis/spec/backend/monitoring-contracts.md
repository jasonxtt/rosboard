...
- `connectedDeviceCount` excludes `routeros:self`, offline terminals, selected traffic interfaces, loopback, and interfaces identified as WAN. An active terminal with no known interface remains countable so missing neighbor metadata does not hide a real device.
...
- Active terminal with an unknown interface -> retain in `connectedDeviceCount`.
...
- Unit: connected LAN device count excludes RouterOS self, offline, WAN, and selected traffic-interface terminals while retaining active unknown-interface terminals.
