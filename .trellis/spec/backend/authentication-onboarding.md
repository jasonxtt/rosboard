...
- Allowed CIDRs are checked before setup/auth. Authenticated write requests must be same-origin; an HTTPS origin terminated by a reverse proxy is accepted when startup-only `trusted_proxy_cidrs` is `true` or the immediate proxy peer matches its CIDR list, supplies one strict forwarded protocol value, and either preserves the public Host or supplies a strict forwarded host value. In trust-all mode, a request without forwarding headers remains a direct request and uses its actual scheme and Host, so direct LAN access keeps working.
