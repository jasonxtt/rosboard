...
- [ ] 15 个视图 + 登录/初始化在新风格下功能完备（与旧前端逐项对齐）。
- [ ] 明暗主题切换无写死色值；移动端 375px 无横向滚动。
- [ ] echarts 依赖移除，uplot 正常工作；`web` 构建产物正确嵌入 `internal/ui/dist`。
- [ ] lint/build 全绿；Go 侧 `go build ./...` 不受影响。
- [ ] 用户在测试机 10.0.0.60 上完成视觉/交互验收后，才谈合并与生产部署（遵守部署验收门）。
