...
1. Extend `internal/config` with a boolean/CIDR-list YAML value, strict CIDR
   validation, and load/save tests.
2. Extend `internal/api.Server` construction with the trust-all flag and any
   parsed trusted proxy ranges.
3. Add strict forwarded-origin resolution, wire it into the API admission
   ...
   scheme.
4. Add API regression tests for trust-all and CIDR modes, HTTP/HTTPS schemes,
