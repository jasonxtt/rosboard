...
- [ ] At meaningful checkpoints inspect exact staged paths/diff for secrets and unrelated changes, commit and push the same branch/Draft PR. Do not merge, mark accepted or archive automatically.
...
Correct presentation in small coherent commits so a faulty slice can be reverted independently after checking later dependencies. Do not reset the branch to the functional reference, force-push, discard other agents' files or remove the frozen preview. Keep the existing safe development proxy.
