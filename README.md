...
这会信任任意直接连接到 rosboard 的来源提交的反代头，用于正确识别反代后的
HTTPS 请求；适合 Caddy 运行在 Docker、rosboard 无法直接看到宿主机地址的
场景。没有反代头的直连请求仍按实际连接的协议和 Host 处理，因此可以同时
使用局域网 IP 直连。使用这个开关前，请通过防火墙或网络策略确保 `8080` 不能
被不可信来源直接访问。省略该字段或设置为 `false`，则保持直连策略。
