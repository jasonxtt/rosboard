...
func TestLoadAndSaveTrustedProxyCIDRList(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	...
	if cfg.TrustedProxyCIDRs.TrustAll || len(cfg.TrustedProxyCIDRs.CIDRs) != 2 || cfg.TrustedProxyCIDRs.CIDRs[0] != "127.0.0.1/32" || cfg.TrustedProxyCIDRs.CIDRs[1] != "2001:db8::/32" {
...
func TestLoadAndSaveTrustAllProxySetting(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte("trusted_proxy_cidrs: true\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if !cfg.TrustedProxyCIDRs.TrustAll || len(cfg.TrustedProxyCIDRs.CIDRs) != 0 {
		t.Fatalf("trust-all proxy setting did not load: %#v", cfg.TrustedProxyCIDRs)
	}
	if err := Save(path, cfg); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(saved), "trusted_proxy_cidrs: true") {
		t.Fatalf("trust-all proxy setting was not persisted: %s", saved)
	}
}

func TestLoadDefaultsTrustedProxySettingToDisabled(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte("trusted_proxy_cidrs: false\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg, err := Load(path)
	if err != nil {
	...
	if cfg.TrustedProxyCIDRs.TrustAll || len(cfg.TrustedProxyCIDRs.CIDRs) != 0 {
		t.Fatalf("false trusted proxy setting should disable proxy trust: %#v", cfg.TrustedProxyCIDRs)
	}
}
	...
	for _, value := range []string{"", "not-a-cidr"} {
		t.Run(value, func(t *testing.T) {
			cfg := Config{
				...
				SampleRetentionHours: 48, TrustedProxyCIDRs: TrustedProxyConfig{CIDRs: []string{value}},
			}
...
func TestLoadRejectsStringTrustedProxySetting(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte("trusted_proxy_cidrs: \"127.0.0.1/32\"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := Load(path); err == nil || !strings.Contains(err.Error(), "trusted_proxy_cidrs must be a boolean or a CIDR list") {
		t.Fatalf("expected trusted proxy type error, got %v", err)
	}
}
