...
export function RoutingRuleWizard({ deviceID, context, rule, egress, onClose, onSaved }: { deviceID: string; context: WizardContext; rule: RoutingRule | null; egress?: Egress | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const generateCurrentPlan = async () => {
    ...
    const proposal: PolicyPlanProposal = {
      egress: draft,
      routingRule: { id: rule?.id ?? '', name: ruleName.trim(), subject: effectiveSubject, ingress: sourceKind === 'legacy' ? trafficIngress : { interfaceLists: [], interfaces: [] }, ...(sourceScope ? { sourceScope } : {}), targetListIds: targetListIDs, egressId: draft.id, priority: Number(rulePriority) || 0, enabled, includeKeywordDomains, revision: rule?.revision ?? 0 },
  ...
  return <>
    ...
    {isPreview ? <PolicyPlanPreview deviceID={deviceID} envelope={plan!} summary={planSummary ?? undefined} onBusyChange={setApplying} onBack={() => setActiveStep(2)} onKeywordBack={() => setActiveStep(1)} onApplied={async () => { await onSaved(); onClose() }} /> : null}
    ...
    {activeStep === 1 ? <><StepLockedNotice locked={stepLocked} /><fieldset className="policy-wizard-fields" disabled={busy || stepLocked}><TargetStep deviceID={deviceID} targetLists={targetLists} targetListIDs={targetListIDs} includeKeywordDomains={includeKeywordDomains} onIncludeKeywordDomains={(value) => { markDraftChanged(); setIncludeKeywordDomains(value) }} onTargetLists={updateTargetLists} onRemoveTarget={(id) => updateTargetLists(targetListIDs.filter((targetID) => targetID !== id))} onPresetPresentation={updatePresetPresentations} onCreateTargetList={setCreatingTargetKind} errors={targetErrors} invalidTargetIDs={invalidTargetIDs} /></fieldset></> : null}
...
function TargetStep({ deviceID, targetLists, targetListIDs, includeKeywordDomains, onIncludeKeywordDomains, onTargetLists, onRemoveTarget, onPresetPresentation, onCreateTargetList, errors, invalidTargetIDs }: { deviceID: string; targetLists: TargetList[]; targetListIDs: string[]; includeKeywordDomains: boolean; onIncludeKeywordDomains: (value: boolean) => void; onTargetLists: (ids: string[]) => void; onRemoveTarget: (id: string) => void; onPresetPresentation: (value: PresetPresentation[]) => void; onCreateTargetList: (kind: 'domain' | 'ip') => void; errors: string[]; invalidTargetIDs: string[] }) {
  return <div className="policy-wizard-stage"><section className="policy-section-card"><h4>访问目标</h4><p className="policy-hint">普通目标列表和应用规则从这里复用。应用规则按域名 / IP 独立准备，Target Library 不会显示 backing rows。</p><TargetSelector deviceID={deviceID} targetLists={targetLists} selectedIDs={targetListIDs} onChange={onTargetLists} onPresetPresentationChange={onPresetPresentation} onCreateTargetList={onCreateTargetList} /><KeywordDomainSetting targetLists={targetLists} targetListIDs={targetListIDs} enabled={includeKeywordDomains} onChange={onIncludeKeywordDomains} />{errors.length ? <PolicyNotice tone="warn" title="访问目标校验未通过"><span>{errors.join('；')}</span>{invalidTargetIDs.length ? <span className="policy-invalid-targets">{invalidTargetIDs.map((id) => <button key={id} type="button" className="link-button" onClick={() => onRemoveTarget(id)}>移除失效目标：{id}</button>)}</span> : null}</PolicyNotice> : null}</section></div>
}
